 ...
 *  Modified by Chortle on 2026-09-01 to accept UCI `position fen` commands.
 *
 **/
!(function (root, name, factory) {
  ...
  const pst = {
    P: [
      0, 0, 0, 0, 0, 0, 0, 0, 78, 83, 86, 73, 102, 82, 85, 90, 7, 29, 21, 44,
      40, 31, 44, 7, -17, 16, -2, 15, 14, 0, 15, -13, -26, 3, 10, 9, 6, 1, 0,
      -23, -22, 9, 5, -11, -10, -2, 3, -19, -31, 8, -7, -37, -36, -14, 3, -31,
      0, 0, 0, 0, 0, 0, 0, 0,
    ],
    N: [
    ...
    toString: function () {
      ...
      if (null == self._str)
        self._str =
          ...
          String(self.kp) +
          "," +
          self.wc.map(String).join(",") +
          "," +
          self.bc.map(String).join(",");
      return self._str;
    },
    gen_moves: function* () {
  ...
  Searcher.prototype = {
    ...
    bound: function (pos, gamma, depth, can_null = true) {
      ...
      //# Table part 2
      if (best >= gamma) self.tp_score[key] = new Entry(best, entry.upper);
      if (best < gamma) self.tp_score[key] = new Entry(entry.lower, best);
      ...
      for (let depth of range(1, maxDepth + 1 /*!ADDED!*/)) {
        ...
        while (lower < upper - EVAL_ROUGHNESS) {
          ...
          yield [
            ...
            gamma,
            score,
            self.tp_move[history[history.length - 1].toString()],
          ];
  ...
  function fromFen(fields) {
    const [placement, color, castling, enPassant] = fields;
    const expanded = placement.replace(/\d/g, (digit) => ".".repeat(Number(digit)));
    const board = Array.from(" ".repeat(21) + expanded.split("/").join("  ") + " ".repeat(21));
    for (let index = 9; index < board.length; index += 10) board[index] = "\n";
    const boardString = board.join("");
    const whiteCastle = [castling.includes("Q"), castling.includes("K")];
    const blackCastle = [castling.includes("k"), castling.includes("q")];
    const ep = enPassant === "-" ? 0 : parse(enPassant);
    let score = 0;
    for (let index = 0; index < boardString.length; index++) {
      const piece = boardString.charAt(index);
      if (isupper(piece)) score += pst[piece][index];
      else if (islower(piece)) score -= pst[piece.toUpperCase()][119 - index];
    }
    const position = new Position(boardString, score, whiteCastle, blackCastle, ep, 0);
    if (color === "w") return [position];
    const rotated = position.rotate();
    return [rotated.rotate(), rotated];
  }
  ...
  sunfish.engine = function (cmd, output = null) {
    ...
    if (args[0] === "uci") {
      ...
      STOPPED = true;
    } else if (args[0] === "position" && args[1] === "fen") {
      hist = fromFen(args.slice(2, 8));
      const movesIndex = args.indexOf("moves", 8);
      const moves = movesIndex === -1 ? [] : args.slice(movesIndex + 1);
      for (let ply of range(moves.length)) {
        const move = moves[ply];
        let i = parse(move.slice(0, 2)),
          j = parse(move.slice(2, 4)),
          prom = move.slice(4).toUpperCase();
        if ((hist.length + ply) % 2 === 0) {
          i = 119 - i;
          j = 119 - j;
        }
      ...
      const think = isFinite(wtime)
        ? 0.8 * Math.min(wtime / 40 + winc, wtime / 2 - 1)
        : Infinity;
